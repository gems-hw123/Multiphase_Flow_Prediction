...
class Crop3D:
    ...
    def __call__(self, input, output, dry_scan, mask):
        h, w, d = input.shape
        ...
        mask = mask[top: top + new_h, left: left + new_w, front: front + new_d]
        return input, output, dry_scan, mask
...
class CTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_path = f'/rds/general/user/hw123/home/data_set/Pore_Voxel_Extraction/seg_dry.pt'
        ...
        if self.transform:
            input_wet_scan, output_wet_scan, dry_scan, mask = self.transform(input_wet_scan, output_wet_scan, dry_scan, mask)
            
        input_two_channel = torch.stack((input_wet_scan,dry_scan))

        return input_two_channel, output_wet_scan.unsqueeze(0), mask.unsqueeze(0)
